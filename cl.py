...
class flet(known):
        ...
        def primitivise(mach, bindings, *body):
                ...
                ## Get the surrounding clambda, which is different from those created below.
                clambda         = lexenv.clambda if lexenv is not nil else make_global_anonymous_clambda()
...
class let(known):
        ...
        def primitivise(mach, bindings, *body):
                ...
                lexenv  = symbol_value(_lexenv_)
                clambda = lexenv.clambda if lexenv else make_global_anonymous_clambda()
...
class function(known):
        ...
        def primitivise(mach, x, name = nil, pydecorators = nil, globalp = nil):
                ...
                lambdap = ir_lambda_p(x)
                lexenv = symbol_value(_lexenv_)
                ## The surrounding clambda.
                clambda = lexenv.clambda if lexenv is not nil else make_global_anonymous_clambda()
                if lambdap:
                        lambda_list, body = x[1][0], x[1][1]
                        return primitivise_lambda(mach, lexenv,
