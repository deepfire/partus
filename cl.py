...
def def_():
        ...
        def binds(name, lambda_list, *body, decorators = []):
                ...
                return { t: { _make_variable_binding(name, t, None) for name in total } }
...
def let():
        def binds(bindings, *body):
                ...
                return { t: { _make_variable_binding(name, t, None) for name, _ in bindings } }
...
def flet():
        def binds(bindings, *body):
                return { t: { _make_function_binding(name, t, None) for name, _, *__ in bindings } }
        def lower(bindings, *body):
