        ...
        def rewrite(orig, bindings, *body):
                ...
                not (listp(bindings)
                     and every(lambda x: (length(x) >= 2 and
                                          listp(second(x))),
                               bindings)) and fail(orig)
                r = (handle_constant_linear_body(body) if all(constantp(x) for x in body) else
                     ...
                     l(_funcall,
                       (lambda rest, tns:
                                l(_labels, xmap_to_conses(lambda tn_b:
                                                                  cons(tn_b[0], tn_b[1][1]),
                                                          list(zip(tns, bindings))),
                                  l(_labels, xmap_to_conses(lambda tn_b:
                                                                    l(tn_b[1][0], l(_rest, rest),
                                                                      l(_apply, l(_function, tn_b[0]), rest)),
                                                            list(zip(tns, bindings))),
                                    *body)))
                       (gensym("REST-"),
                       (gensym("FN-" + symbol_name(x[0]) + "-")
                        for x in vectorise_linear(bindings)))))
                dprintf("FLET r %s %s %s -->\n%s",
                        all(constantp(x) for x in body), not (bindings or body), not bindings,
                        pp_consly(r))
                return t, r
...
class labels(known):
        def binder(exp, further):
                ...
                dprintf("LABING: %s", " ".join(pp_consly(b) for b in bindings))
                with progv({ _walker_lexenv_:
                              make_lexenv(env, allocate_tns = symbol_value(_walker_allocate_tns_),
                                          name_funcframe = { dprintf("LABIN: processing (%s %s %s)",
                                                                     name, pp_consly(lam), pp_consly(body)) or
                                                             name: function_binding(name, _function, fn(name, lam))
...
class lambda_(known):
        ...
        def nvalues(*_):            return 1
        def nth_value(n, orig, *_): return orig if n is 0 else nil
        def lower(lambda_list, *body, name = nil, decorators = nil, globalp = nil):
        ...
        def rec(x):
                ...
                if x is nil or isinstance(x, symbol_t) and not constantp(x):
                        ...
                        if isinstance(x[0], symbol_t):
                                ...
                                if not hasattr(known, "lower"):
                                        error("It is not KNOWN to me, how to lower %s forms.", form[0])
                                res = known.lower(*vectorise_linear(form[1]), **alist_hash_table(args))
