...
def text_ast(text):
        return pyb.compile(text, "", 'exec', flags = ast.PyCF_ONLY_AST).body
...
class setq(known):
        ...
        def primitivise(mach, name, value):
                ...
                if not lexical_binding or lexical_binding.kind is _special:
                        ...
                        return p.special_setq(p.name(unit_symbol_pyname(name)), primitivise(mach, value))
...
def emit_python_ast(prim) -> [ast.AST]:
        asts = py.help_prog([prim])
...
def emit_cfg(prim):
        gdata   = tri.unit_global_data_t()
        return tri.primitive_cfg(symbol_value(_machine_), gdata, prim)

def lower(form, lexenv = nil, machine = None):
        ...
        with target_machine(machine):
                ...
                if machine is pymach:
                        ##
                        ## LIR -> Python AST
                        ast  = emit_python_ast(prim)
                        ...
                        return ast
                if machine is cfgmach:
                        ##
                        ## LIR -> CFG
                        cfg = emit_cfg(prim) # Ignored.
                        return cfg
        error("In LOWER %s: unknown or unspecified machine: %s.", pp_consly(form), machine)
...
def compilation_unit_prologue(funs, syms, gfuns, gvars):
        ...
        l = list_
        def import_prologue():
                return py.help(pymach.import_("cl"))[0]
        def ir_literal_pylist(xs):
                return l(_primitive, py.pylist, *xs)
        def symbol_prologue():
                def wrap(x):
...
def assemble(_ast: [ast.stmt], form: cons_t, filename = "") -> "code":
        ...
        bytecode = pyb.compile(ast.fix_missing_locations(ast_module(_ast)), filename, "exec")
...
def eval_python(expr_or_stmt):
        ...
        call = ast.fix_missing_locations(ast_module(
                        ...
                        if exprp else
                        [expr_or_stmt]))
        code = handler_case(lambda: pyb.compile(call, "", "exec"),
                            (error_t,
                             lambda cond:
