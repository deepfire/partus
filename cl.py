...
class flet(known):
        ...
        def lower(bindings, *body):
                ...
                fbindings       = [ function_binding(b[0], _function, fn(b[0], b[1][0])) for b in bindings ]
                for b in fbindings:
                        b.allocate_tn()
                # dprintf("fbindings: %s", fbindings)
                return p.funcall(p.lambda_(([ b.tn for b in fbindings ], [], [], None, [], [], None),
                                           with_lexenv(make_lexenv(lexenv, ## TNs were just preallocated.
                                                                   name_funcframe = { b.name: b for b in fbindings }),
                                                       lambda: p.progn(*(primitivise(x) for x in body)))),
