...
def boot(set, boot, on_unboot = None):
        def definer(orig):
                def unboot():
                        _frost.setf_global(orig, orig.__name__, globals(), force = True)
                        if on_unboot:
                                on_unboot()
...
def _do_pyimport_symbol(symbol, globals, name_xform = _frost.lisp_symbol_name_python_name, force = False):
        inmod_name = name_xform(symbol_name(symbol))
        # _debug_printf("PYMPORT %s", inmod_name)
        _frost.setf_global(symbol, inmod_name, globals, force = force)

def _pyimport_symbol(symbol, globals = None, name_xform = _frost.lisp_symbol_name_python_name, force = False):
        _do_pyimport_symbol(_boot_check_type(symbolp, symbol), _defaulted(globals, _py.globals()), name_xform, force)

def _global_pyname(name):
        if name[0] != "*" != name[-1]:
...
def _symbol_pyname(name):
        return "__" + name.replace("-", "_").strip("_%").lower()

def _intern_and_bind_symbols(*names, globals = None):
        globals = _defaulted(globals, _py.globals())
        for name in names:
                _pyimport_symbol(_intern(name)[0], globals, _symbol_pyname)

def _intern_and_bind_globals(*names, globals = None):
        globals = _defaulted(globals, _py.globals())
        for name in names:
                _pyimport_symbol(_intern(name)[0], globals, _global_pyname)
...
def _boot_symbolicate_global_dynamic_scope():
        def upgrade_scope(xs):
                ...
                for k, v in kvs:
                        ...
                        xs[sym] = v
                        _do_pyimport_symbol(sym, globals(), force = t)
...
def import_(symbols, package = None, populate_module = True):
        p = _coerce_to_package(package)
        symbols = _vectorise_linear(_ensure_list(symbols))
...
def _symbolicate(x, name, slot, globals):
        ...
        _frost.setf_global(sym, pyname, globals, force = t)
...
class _progv():
        ...
        #         return _withless() if not args[0] else object.__new__(cls)
...
class _dynamic_scope():
        "Courtesy of Jason Orendorff."
        def let(self, **keys):
                return _progv(keys)
        def maybe_let(self, p, **keys):
                return _progv(keys) if p else None
...
def _block(nonce_or_fn, body = None):
...
of nonce-ing is to be handled manually."""
        if not body: # Assuming we were called as a decorator..
                ...
                return _catch(nonce_or_fn, body)
                ...
                for type, handler in cluster:
                        if not isinstance(type, str):
                                if isinstance(cond, type):
                                        ...
                                        with _progv({ _handler_clusters_: handler_clusters[:-(n + 1)]}):
                                                handler(cond)
        return nil
...
_intern_and_bind_globals("*DEBUG-CONDITION*", "*DEBUG-RESTARTS*", "*NESTED-DEBUG-CONDITION*")
...
def _invoke_debugger(condition):
        ...
        with _progv({_debug_condition_: condition,
                ...
                try:
                        ...
                        with _progv({ _standard_output_: _symbol_value(_standard_output_),
...
def _deftype(type_name_or_fn, globals = None):
        def do_deftype(fn, type_name = type_name_or_fn):
                ...
                old_global_name = (type_name_or_fn.__name__ if functionp(type_name_or_fn) else
                                   fn.__name__)
                globals = _defaulted(globals, _py.globals())
                old_global = (_global(old_global_name, globals)
...
for fn  in __boot_defunned__:   _frost.setf_global(defun(fn),     fn.__name__,  globals(), force = t)
for cls in __boot_defclassed__: _frost.setf_global(defclass(cls), cls.__name__, globals(), force = t)
doit = True
...
_intern_and_bind_symbols("%MV-MARKER")

def _values_frame(*xs):
        return (__mv_marker,) + xs

def _values_frame_p(x):
        return isinstance(x, tuple) and x[0] is __mv_marker
...
_intern_and_bind_globals("*PRINT-ARRAY*", "*PRINT-BASE*", "*PRINT-CASE*", "*PRINT-CIRCLE*",
...
"""
        with _progv(__standard_io_syntax__):
                return body()
...
def _caller_args(n = 0):
        return _frame_locals(_sys._getframe(n + 2))
...
SETF of MACRO-FUNCTION."""
        ...
        if b_or_res and _bindingp(b_or_res) and consp(b_or_res.value):
               lambda_list, body = b_or_res.value
               b_or_res = _compile_in_lexenv(list_(lambda_, lambda_list, *body),
                                             lexenv = environment)
...
def _define_global_sym_for_pyname(globals, pyname, sym):
...
def _set_function_definition(globals, x, lambda_expression = None, check_redefinition = nil):
        ...
        def do_set_function_definition(function):
                if not identity_redef and function:
                        ...
                        _define_global_sym_for_pyname(globals, forigname, x)
                        _frost.setf_global(function, _ensure_function_pyname(x), globals)
                return function
...
(__load_toplevel, __compile_toplevel, __execute) = [ _keyword(x)  for x in [ "LOAD-TOPLEVEL", "COMPILE-TOPLEVEL", "EXECUTE" ] ]
(__load, __compile, __eval)                      = [ intern(x)[0] for x in [ "LOAD", "COMPILE", "EVAL" ] ]
...
_intern_and_bind_globals("*DEFAULT-PATHNAME-DEFAULTS*")
...
def _pp_consly(x, dispatch = dict(), str_printer = repr, max_depth = 4):
        if consp(x):
                ...
                return "{ " + ", ".join("%s: %s" % (k, _pp_consly(v, dispatch, str_printer, max_depth))
                                        for k, v in x.items()) + "}"
...
def length(x):
        if listp(x):
                ...
                _not_implemented("LENGTH: non-list case on object %s (of type %s)" % (x, type(x).__name__))
...
_intern_and_bind_symbols("LIST", "APPEND", "QUOTE", "QUASIQUOTE", "COMMA", "SPLICE")
...
def _expand_quasiquotation(form):
        ...
        def process_form(x):
                return ((malform(x) if len(x) != 2 and x[0] is __quasiquote else
                         process_qq(x[1]))        if isinstance(x, tuple)   else
                        x                         if atom(x)                else
                        mapcar(process_form, x))
        def process_qq(x):
                if atom(x):
                        return list_(__quote, x)
                else:
                        acc = [__append]
                        ptr = x
                        while ptr:
                                ...
                                def do_one(x):
                                        return list_(__list,
                                                     (list_(__quote, x) if atom(x) else
                                                      process_qq(x)))
                                if isinstance(ptr, tuple):
                                        xi = ptr
                                        if len(xi) != 2 or xi[0] not in (__quasiquote, __comma, __splice):
                                                error("Invalid reader output, bad tuple intermarker: %s", _pp_consly(form))
                                        if xi[0] is splice:
                                                error(",@ after dot in %s", _pp_consly(form))
                                        acc.append(process_qq(xi[1]) if xi[0] is __quasiquote else
                                                   xi[1])            #           comma
                                        break
                                xi = ptr[0]
                                if isinstance(xi, tuple):
                                        if len(xi) != 2 or xi[0] not in (__quasiquote, __comma, __splice):
                                                error("Invalid reader output, bad tuple intermarker: %s", _pp_consly(form))
                                        acc.append(do_one(process_qq(xi[1])) if xi[0] is __quasiquote else
                                                   list_(__list, xi[1])      if xi[0] is __comma      else
                                                   xi[1])                    #           splice
                                else:
...
def _run_tests_quasiquotation():
        ...
        assert(_runtest(quasiquotation_simple,
                        ## `(1 ,2 3 ,@4 5 (,6 ,@7) ,@8 ,@9)
                        (__quasiquote, list_(1, (__comma, 2), 3, (__splice, 4), 5,
                                             list_((__comma, 6), (__splice, 7)),
                                             (__splice, 8), (__splice, 9))),
                        ## (append (list (__quote 1)) (list 2) (list (__quote 3)) 4 (list (__quote 5))
                        ##         (list (append (list 6) 7)) 8 9)
                        list_(__append, list_(__list, list_(__quote, 1)), list_(__list, 2),
                              list_(__list, list_(__quote, 3)), 4,
                              list_(__list, list_(__quote, 5)), list_(__list, list_(__append, list_(__list, 6), 7)),
                              8, 9),
                        printer = _pp_consly))
        ...
        assert(_runtest(quasiquotation_nested,
                        ...
                        (__quasiquote,
                         list_(1, (__comma, 2), (__splice, 3),
                               (__quasiquote, list_(4, (__comma, (__comma, 5)),
                                                       (__splice, 6), (__splice, (__comma, 7)))))),
                        list_(__append,
                              list_(__list, list_(__quote, 1)), list_(__list, 2), 3,
                              ## The first pass ought to be:
                              ## (__append, (__list, (__quote, 4)), (__list, (__comma, 5)), 6, (__comma, 7))
                              list_(__list, list_(__list,
                                                  list_(__quote, __append),
                                                  list_(__list, list_(__quote, __list), list_(__list, list_(__quote, __quote), list_(__quote, 4))),
                                                  list_(__list, list_(__quote, __list), 5),
                                                  list_(__quote, 6),
                                                  7))),
                        printer = _pp_consly))

if _getenv("CL_RUN_TESTS") != "nil":
        with _progv({ _reader_trace_qqexpansion_: nil }):
...
_intern_and_bind_globals("*STACK-TOP-HINT*", "*TRACEBACK*", "*SIGNALLING-FRAME*")
...
def __cl_condition_handler__(condspec, frame):
        ...
        def continuation():
                ...
                if type_of(cond) not in __not_even_conditions__ and isinstance(cond, condition_t):
                        ...
                        with _progv({_traceback_: traceback,
...
def handler_bind(fn, *handlers, no_error = identity):
        ...
        if _frost.pytracer_enabled_p() and _frost.tracer_hook("exception") is __cl_condition_handler__:
                ...
                with _progv({_handler_clusters_: (_symbol_value(_handler_clusters_) +
...
def _restart_bind(body, restarts_args):
        with _progv({_restart_clusters_: (_symbol_value(_restart_clusters_) +
...
def _read_ast(x):
        ...
        with _progv(# {_read_case_: _keyword("preserve")}
                   {}):
...
_intern_and_bind_globals("*BOUND-FREE-RECURSOR*")
...
def _ast_bound_free(astxs):
        def ast_rec(astxs):
                ...
                return _separate(3, bound_free, [ x for x in (astxs if isinstance(astxs, list) else [astxs])
                                                  if x is not None ])
        with _progv({_bound_free_recursor_: ast_rec}):
...
_intern_and_bind_globals("*SEGMENT-ITERATION*")

_intern_and_bind_symbols("%SOME", "%MAYBE", "%OR", "%FUNCHER",
                         "IR-ARGS",
                         "LAMBDA")
...
def _define_funcher(name, *pattern): __funchers__[name] = _xmap_to_conses(_preprocess_metasex, pattern)
def _funcher_name_p(x):              return isinstance(x, symbol_t) and x in __funchers__
def _find_funcher(x):                return __funchers__[x]
...
class _matcher():
        ...
        def test(m, test, desc, bound, name, exp, resf:"() -> result", fail_pat,
                ...
                with _match_level(([desc + ":", test] if desc else [test])):
        ...
        def simplex(m, bound, name, exp, pat, orifst):
                ...
                #         if exp_pat_hit:
                #                 return exp_pat_hit
                # _debug_printf("mtd.method: %s  exp %s  pat %s", mtd.method, _pp_consly(exp), _pp_consly(pat))
        ...
        def initialise_cache(m):
                ...
                for mr in m.__complex_patterns__.values():
                        mr.cache = _collections.defaultdict(dict)
        def __init__(m):
                # _without_condition_system(_pdb.set_trace)
                # _debug_printf("__some is: %s", __some)
                #
                ## Yields:
                #
                # (Pdb) dis.disassemble(_sys._getframe(11).f_code)
                # 6440           0 LOAD_GLOBAL              0 (_without_condition_system) 
                #               3 LOAD_GLOBAL              1 (_pdb) 
                #               6 LOAD_ATTR                2 (set_trace) 
                #               9 CALL_FUNCTION            1 
                #              12 POP_TOP              
                #
                # 6441          13 LOAD_GLOBAL              3 (_debug_printf) 
                #              16 LOAD_CONST               1 ('__some is: %s') 
                #              19 LOAD_GLOBAL              4 (_matcher__some) 
                #              22 CALL_FUNCTION            2 
                #              25 POP_TOP              
                _without_condition_system(_pdb.set_trace)
                _debug_printf("__some is: %s", __some)
                m.__simplex_patterns__, m.__complex_patterns__ = dict(), dict()
                m.register_complex_matcher(__some,  m.segment)
                m.register_complex_matcher(__maybe, m.maybe)
                m.register_complex_matcher(__or,    m.or_)
                m.match_calls = 0
        ...
        def segment(m, bound, name, exp, pat, orifst, aux, limit, end = None):
                ...
                with _match_level(trace_args):
                 with _progv({ _segment_iteration_: (0 if firstp else (symbol_value(_segment_iteration_) + 1)) }):
        ...
        def match(m, bound, bname, exp, pat, orifst, aux, limit, name = "MATCH"):
                ...
                #         _debug_printf("MATCH %s", _pp_consly(exp))
                #         _debug_printf("PATT- %s", _pp_consly(pat))
                m.match_calls += 1
                ...
                with _match_level([exp, ("pat", pat), ("first", orifst[1])], name = name):
                 return \
                     _r(m.test(exp == pat, "atom match", bound, bname, exp, lambda: m.prod(exp, orifst[0]), pat)
...
__metasex_pp_words__       = { __newline, __indent, __lead, __notlead, __nottail }
__metasex_bind_words__     = { __binder, __bind, __bound }
__metasex_leaf_words__     = { __form,
                               __newline, __indent,
                               __for_matchers_xform, __for_not_matchers_xform }

def _metasex_word_p(x):      return isinstance(x, symbol_t) and x in __metasex_words__
def _metasex_pp_word_p(x):   return isinstance(x, symbol_t) and x in __metasex_pp_words__
def _metasex_bind_word_p(x): return isinstance(x, symbol_t) and x in __metasex_bind_words__
...
def _preprocess_metasex(pat):
        ...
        def _consify_metasex(x):
                return (_xmap_to_conses(_consify_metasex, x)                 if isinstance(x, tuple) else
                        ## %SOME preprocessing is special-cased here, since it is the only reliable place to do it..
                        list__(__some, _xmap_to_conses(_consify_metasex, x)) if isinstance(x, list)  else
                        { k: _consify_metasex(v)
                          for k, v in x.items() }                            if isinstance(x, dict)  else
                        x)
        def rec(x):
                l = list_
                # _debug_printf("rec  %s", _pp_consly(x))
                def prep_binding(b):
                        k, v = tuple(b.items())[0]
                        return { k: rec(v)[0] }
                def prep_linear(xs):
                ...
                return (l(prep_binding(x))             if isinstance(x, dict)                 else
                        l(l(__indent, 1))              if x == " "                            else
                        l(l(__newline, 0))             if x == "\n"                           else
                        l(l(__newline, x))             if isinstance(x, int)                  else
                        l(x)                           if atom(x)                             else
                        (error("Reference to an undefined funcher %s in pattern %s.",
                               x[1][0], pat) if not _funcher_name_p(x[1][0]) else
                         _find_funcher(x[1][0]))      if x[0] is __funcher                    else
                        l(prep_linear(x)))
        consified = _consify_metasex(pat)
        ret = rec(consified)[0]
        # _debug_printf("Preprocessed\n%s  ->\n%s  ->\n%s", pat, _pp_consly(consified), _pp_consly(ret))
        return ret

def _strip_metasex(form, strip_pp = nil, strip_bind = nil):
        def de_bind(x):
                if x[0] is __bound:   return list_(x[1][0])
                if x[0] is __bind:    return list_(x[1][1][0])
                if x[0] is __binder:  return nil
                ## Yet Another Python Bug IFEXPR-BREAKS-EVALUATION-CONTRACT
                return (list_(x[1][0])    if x[0] is __bound                             else
                        list_(x[1][1][0]) if x[0] in __bind                              else
                        nil               if x[0] is __binder                            else
                        error("Invalid form for binder stripping: %s", _pp_consly(x)))
        ret = mapcon(lambda x: (list_(x[0])                          if not consp(x[0])                                  else
                                list_(_strip_metasex(
                                        x[0], strip_pp, strip_bind)) if not (symbolp(x[0][0]) or strip_pp or strip_bind) else
                                nil                                  if (strip_pp
                                                                         and _metasex_pp_word_p(x[0][0]))                else
                                _strip_metasex(de_bind(x[0]), strip_pp, strip_bind)
                                                                     if (strip_bind
                                                                         and _metasex_bind_word_p(x[0][0]))              else
                                list_(_strip_metasex(x[0], strip_pp, strip_bind))),
                      form)
        # _debug_printf("STRIP: %s\n--->\n%s", _pp_consly(form), _pp_consly(ret))
        return ret
...
_string_set("*METASEX-KIND*", "metasex")

def _form_metasex(form, kind = "metasex"):
        ...
        return (_preprocess_metasex((__typep, t))    if not consp(form)                                                  else
                getattr(_find_known(form[0]), kind)  if isinstance(form[0], symbol_t) and _find_known(form[0])           else
                _preprocess_metasex((([(__notlead, " "), (__form,)] if kind == "metasex_pp" else
                                      [                  (__form,)]),)))
...
def _combine_pp(f0, fR, originalp, consdotp):
        def orig_tuple_comb(body):
                ...
                with _progv({ _pp_base_depth_: new_base }):
                        ret = body(new_base)
                        return None if ret is None else ("(" + ret + ")")
        def body(base):
                f0r = f0()
                if f0r is None: return
                with _progv({ _pp_depth_: base + len(f0r.split("\n")[-1]) }):
                        fRr = fR()
...
_intern_and_bind_symbols("%SATISFIES", "%CONS", "%TYPEP")

class _metasex_matcher(_matcher):
        def __init__(m):
                _matcher.__init__(m)
                m.register_simplex_matcher(__form,             m.form)
                m.register_simplex_matcher(__satisfies,        m.satisfies)
                m.register_simplex_matcher(__cons,             m.cons)
                m.register_simplex_matcher(__typep,            m.typep)
        @staticmethod
...
class _metasex_mapper(_metasex_matcher):
        ...
        def form(m, bound, name, form, pat, orifst, ignore_args = None):
                # _debug_printf("MM.FORM %s", _pp_consly(form, max_depth = 10))
                handled, ret = m.process_formpat_arguments(form, pat) if not ignore_args else (None, None)
                if handled:
                        return m.succ(bound, ret)
                def cont(xformed):
                        return _metasex_matcher.form(m, bound, name, xformed, pat, orifst, ignore_args = ignore_args)
                return symbol_value(_mapper_fn_)(form, cont)

_mapper = _metasex_mapper()

def _map_sex(fn: "Form -> (Form -> ({} Form Bool)) -> ({} Form Bool)",
             sex, matcher = _mapper, start_inside = nil) -> "({} Form Bool)":
        with _progv({ _mapper_fn_: fn }):
                return _match(matcher, sex, _form_metasex(sex) if start_inside else list_(_form))

def _xform_ir(fn: "Form -> (Form -> ({} Form Bool)) -> ({} Form Bool)",
              sex, matcher = _mapper, start_inside = nil) -> "Form":
        with _matcher_pp_stack():
                _, r, f = _map_sex(fn, sex, matcher, start_inside)
        if f is not None:
...
_intern_and_bind_symbols("%NEWLINE", "%INDENT", "%LEAD", "%NOTLEAD", "%NOTTAIL")

class _metasex_matcher_pp(_metasex_matcher):
        def __init__(m):
                _metasex_matcher.__init__(m)
                m.register_complex_matcher(__newline,     m.newline)
                m.register_complex_matcher(__indent,      m.indent)
                m.register_complex_matcher(__lead,        m.lead)
                m.register_complex_matcher(__notlead,     m.notlead)
                m.register_complex_matcher(__nottail,     m.nottail)
        @staticmethod
        ...
        def newline(m, bound, name, exp, pat, orifst, aux, limit):
                ...
                with _progv({ _pp_depth_:      new_base,
        ...
        def indent(m, bound, name, exp, pat, orifst, aux, limit):
                ...
                with _progv({ _pp_depth_: new_depth }):
...
def _pp_sex(sex, strict = t, initial_depth = None):
        ...
        pat = _form_metasex(sex, kind = "metasex_pp")
        with _progv({ _pp_depth_:         initial_depth,
...
def _mock(sex, initial_depth = None, max_level = None):
        ...
        def mock_complexes(xs, new_level):
                with _progv({ _pp_base_depth_: symbol_value(_pp_base_depth_) + 2 }):
                ...
                if level > max_level:
                        ...
                        if atom(car):
                                ...
                                return (mock_complexes(sex, level + 1) if level < max_level else
                                        "..more..")
        initial_depth = _defaulted_to_var(initial_depth, _pp_base_depth_)
        with _progv({ _pp_base_depth_: initial_depth }):
                return rec(sex, 0)
...
_intern_and_bind_symbols("&WHOLE", "&OPTIONAL", "&REST", "&BODY", "&KEY", "&ALLOW-OTHER-KEYS", "&AUX", "&ENVIRONMENT")

__lambda_words__ = { _whole, _optional, _rest, _body, _key, _allow_other_keys, _aux, _environment }

def _lambda_word_p(x):
        return x in __lambda_words__

_intern_and_bind_symbols("DECLARE")
...
_eval_when_ordered_legacy_keywords = compile, load, eval
_eval_when_keywords = set(_eval_when_ordered_keywords) | set(_eval_when_ordered_legacy_keywords)
def _parse_eval_when_situations(situ_form):
        ...
        if not (listp(situ_form) and not (set(situ_linear) - _eval_when_keywords)):
                error("In EVAL-WHEN: the first form must be a list of following keywords: %s.", _eval_when_keywords)
        return [ (new    in situ_linear or
                  legacy in situ_linear)
                 for new, legacy in zip(_eval_when_ordered_keywords,
                                        _eval_when_ordered_legacy_keywords) ]
...
__known_trace_args__ = { "forms", "macroexpanded", "rewrites", "knowns", "primitives", "ast", "module_ast", "bytecode",
                         "toplevels",
                         "subknowns", "inner_knowns", "known_choices", "known_primitives",
                         "pretty_full" }
...
def _sex_deeper(n, body):
        with _progv({ _pp_base_depth_: _pp_base_depth() + n }):
                return body()
...
class _variable_binding(_variable, _binding):
        ...
        def allocate_tn(self):
                ...
                self.tn = _variable_tn(self.name)
class _function_binding(_function, _binding):
        ...
        def allocate_tn(self):
                ## This calls onto primitive IR
                self.tn = _function_tn(self.name)
...
def _gotag_bindingp(x):    return isinstance(x, _gotag_binding)

# Lexenv

_intern_and_bind_globals("*LEXENV*")
_intern_and_bind_symbols("NULL")
        ...
        def __init__(self, parent = nil, clambda = None,
                ...
                def complete_name_frame(framespec):
                        ...
                        for binding in framespec.values():
                                res[binding.kind].add(the(_binding, binding))
                        return res
                def complete_kind_frame(framespec):
                        ...
                        for set_ in framespec.values():
                                for binding in set_:
                                        names[binding.name] = the(_binding, binding)
                        return res
                ...
                if not (parent or clambda):
                        error("Asked to create a lexenv without provision of either a clambda or a parent lexenv.")
                self.clambda  = _defaulted(clambda, (self.parent.clambda if parent is not null else
                                                     nil))
...
def _make_lexenv(parent = nil, clambda = None, **initial_content):
        """ :PARENT - NULL for a null lexenv, nil for the value of *LEXENV*.
            :{NAME,KIND,FULL}-{VAR,FUNC,BLOCK}FRAME - constituents."""
        return _lexenv(parent, clambda = clambda, **initial_content)

__the_null_lexenv__ = _make_null_lexenv(nil)
def _the_null_lexenv():
        return __the_null_lexenv__

def _make_lexenv_varframe(names, parent = nil, clambda = None, forms = _repeat(None), allocate_tns = nil):
        env = _make_lexenv(parent = parent, clambda = clambda,
                           kind_varframe  = { variable: { _variable_binding(sym, variable, form)
                                                          for sym, form in zip(names, forms) } })
        allocate_tns and env.allocate_tns()
        return env

def _make_lexenv_funcframe(bindings, parent = nil, clambda = None, allocate_tns = nil):
        env = _make_lexenv(parent = parent, clambda = clambda,
                           kind_funcframe = { function: { _function_binding(sym, function, _fn(sym, clam.lambda_list))
...
_string_set("*WALKER-LEXENV*", nil)        ## This is for regular macro expansion.
_string_set("*WALKER-CLAMBDA*", nil)       ## Should the original lexenv and clambda be merged into *WALKER-ARGS*?
                                           ## Or should we, still better, add argument passing to the matcher?
_string_set("*WALKER-BINDER*", nil)
_string_set("*WALKER-BINDER-ARGS*", nil)

_define_funcher(__lambda, (__binder, _lambda),
                " ", ([(__notlead, " "), (__or, (__satisfies, _lambda_word_p),
                                          (__bind, variable, (__satisfies, _namep)),
                                          (__bind, variable, ((__satisfies, _namep), (__bound, (__form,)))))],),
                1, [(__notlead, "\n"), (__bound, (__form,))])

## Unregistered Issue LEXENV-WALKER-NOT-REENTRANT->MACROEXPANSION-DAMAGED-GOODS
class _lexenv_walker(_metasex_mapper):
        def __init__(m):
                _metasex_mapper.__init__(m)
                m.register_complex_matcher(__binder,        m.establish_binder)
                m.register_simplex_matcher(__bind,          m.add_binding)
                m.register_simplex_matcher(__bound,         m.setup_lexenv)
        def establish_binder(m, bound, name, exp, pat, orifst, aux, limit):
                return pat[0][1][0].binder(exp, lambda form: m.match(bound, name, exp, pat[1], orifst, aux, limit))
        def add_binding(m, bound, name, exp, pat, orifst):
                ...
                position = _segment_iteration()
                if isinstance(binder.args, int):
                        binder.args = position + 1
                for i in range(position, len(binder.positionally)):
                ...
                if res[2] is not None: ## Propagate match failure.  Can we incrementalise the above bustage here?
                        return res
                ## Success -- extend the pre-lexenv with the binding.
                name, what = (first(exp), second(exp)) if listp(exp) else (exp, nil)
                binder.positionally[position] = (kind, name, what)
                return ret
        def setup_lexenv(m, bound, name, exp, pat, orifst):
                ...
                if not binder.positionally:
                        return further()
                kind = binder.positionally[0][0]
                names, forms = zip(*((x[1], x[2])
                                     for x in (binder.positionally[i]
                                               for i in range(len(binder.positionally)))))
                space, bctor = (("name_varframe",   _variable_binding) if kind in (variable, symbol_macro) else
                                ("name_funcframe",  _function_binding) if kind in (macro, function)        else
                                ("name_blockframe", _block_binding)    if kind is block                    else
                                ("name_gotagframe", _gotag_binding)    if kind is gotag                    else
                ...
                with _progv({ _walker_lexenv_: _make_lexenv(env,
                                                            **{ space: { ctor(name, kind, what )
                                                                         for name, what in zip(names, whats) } }) }):
                        return further()

_lexenv_walker = _lexenv_walker()

def _walk_with_lexenv(fn: "Form -> (Form -> ({} Form Bool)) -> ({} Form Bool)",
                      sex, lexenv = nil) -> "Form":
        with _progv({ _walker_lexenv_: _coerce_to_lexenv(lexenv),
                      _metasex_kind_:  "metasex_bind" }):
                return _xform_ir(fn, sex, matcher = _lexenv_walker)
...
def _compiler_defun(name: symbol_t, lambda_expression: cons_t, check_redefinition = t) -> bool:
        ...
        _set_global_function(name, _function(name, function, lambda_expression = lambda_expression))
        return nil
...
def _compiler_defmacro(name, lambda_expression, check_redefinition = t):
        ...
        if check_redefinition and isinstance(name, symbol_t):
                if oldef and oldef.kind is function:
                        ...
                        _warn_possible_redefinition(oldef.name, name)
        _set_global_function(name, _function(name, macro, lambda_expression = lambda_expression))
        return nil
...
def _define_state_tracker(name):
        macro = """
_string_set("*COMPILER-%sP*", nil)

def _%sp(): return _symbol_value(_compiler_%sp_)

_%s          = _defwith("_%s",
                          lambda *_: _dynamic_scope_push({ _compiler_%sp_: t }),
                          lambda *_: _dynamic_scope_pop())
_no_%s       = _defwith("_no_%s",
                          lambda *_: _dynamic_scope_push({ _compiler_%sp_: nil }),
                          lambda *_: _dynamic_scope_pop())
"""
        exec(macro % ((name.upper(),) + (name,) * 8))

_define_state_tracker("linear")
  ...
  (with-compilation-unit (:policy nil)
    ...
    (load \"foo.lisp\"))"""
        ...
        if _symbol_value(_in_compilation_unit_) and not override:
                try:
                        with _progv({_top_compilation_unit_p_: nil}):
                                ret = fn()
                        succeeded_p = t
                        return ret
                finally:
                ...
                id = gensym(id)
                with _progv({_compiler_error_count_: 0,
...
def _fop_make_symbol_available(globals, package_name, symbol_name,
        ...
        if function_pyname is not None:
                ...
                if gfunp:
                        _frost.setf_global((symbol_function(symbol)
        ...
        if gvarp:
                if _find_global_variable(symbol):
                        ...
                        assert(value is not None)
                        _frost.setf_global(value, _unit_variable_pyname(symbol), globals)
                else:
        ...
        if symbol_pyname is not None:
                ...
                _frost.setf_global(symbol, symbol_pyname, globals)
...
_intern_and_bind_symbols("DEFMACRO")
...
# ((intern("DEFMACRO")[0], " ", (_satisfies, _namep), " ", ([(_notlead, " "), (_satisfies, _namep)],),
#   1, [(_notlead, "\n"), (_bound, _form)]))
def DEFMACRO(name, lambda_list, *body):
        l, l_ = list_, list__
        return l(__eval_when, l(_compile_toplevel, _load_toplevel, _execute),
                 ...
                 l(__ir_args,
                   ...
                   l("decorators", _ir_cl_call("_set_macro_definition", _ir_funcall("globals"),
                                               l(__quote, name),
                                               l(__quote, nil # l_(name, lambda_list, _consify_linear(body))
                                                 ))),
                   ["name", name]))
...
_string_set("*MACROEXPANDER-COMPILERP*", nil)

def _macroexpander_xform(form, further):
        expanded_form, expandedp = macroexpand(form, _walker_lexenv(), compilerp = symbol_value(_macroexpander_compilerp_))
        # if expandedp:
        #         _debug_printf("\n%s\n%s\n->\n%s",
        #                       "expanded" if expandedp else "intact",
        #                       _pp_consly(form),
        #                       _pp_consly(expanded_form))
        return further(expanded_form)

def macroexpand_all(sex, lexenv = nil):
        return _walk_with_lexenv(_macroexpander_xform, sex, lexenv)

def _compiler_macroexpand_all(form, lexenv = nil):
        with _progv({ _macroexpander_compilerp_: t }):
...
def defknown(metasex_or_class, name = None):
        def do_def(cls, sym, pyname, metasex):
                orig = _global(pyname, globals())
                rewrite, lower = [ getattr(cls, x, None) for x in ["rewrite", "lower"] ]
                ## Complete, record the deeds.
                metasex = _preprocess_metasex(metasex)
                cls.name           = sym
                cls.metasex        = _strip_metasex(metasex, strip_pp = t, strip_bind = t)
                cls.metasex_pp     = _strip_metasex(metasex,               strip_bind = t)
                cls.metasex_bind   = _strip_metasex(metasex, strip_pp = t)
                cls.rewrite_params = _mapset(lambda x: x[0], _function_lambda_list(rewrite)[3])
                if lower:
                        cls.lower_params = _mapset(lambda x: x[0], _function_lambda_list(lower)[3])
                sym.known         = cls
                _define_global_sym_for_pyname(globals(), pyname, sym)
                return orig # pass through -- let the symbol be bound to the name
        def _def(cls, name = name, metasex = metasex_or_class):
                pyname = cls.__name__
                sym     = intern(_frost.python_name_lisp_symbol_name(pyname))[0]
        ...
        return (_def(metasex_or_class, metasex = None) if isinstance(metasex_or_class, type)    else
                _def                                   if isinstance(metasex_or_class, tuple)   else
...
def _rewrite_1(cont: "Form -> ({} Form Bool)", form) -> "(Bool ({} Form Bool))":
        "All non-atom forms are interpreted as knowns at this point."
        if atom(form):
                return nil, cont(form)
        known = _find_known(form[0])
        if not known:
                error("Unknown form encountered at rewrite stage: %s", _pp_consly(form))
        # _debug_printf("%s(%s) calling %s.REWRITE(cont, %s, ...)", _caller_name(3), _pp_consly(_caller_args(3)), known, _pp_consly(form, max_depth = 3))
        args = _vectorise_linear(form[1])
        if not known.rewrite.__code__.co_flags & 4 and len(args) != known.rewrite.__code__.co_argcount - 2:
                error("Invalid form: %s -- %d arguments provided, but %s.REWRITE expects only %d.",
                      _pp_consly(form), len(args), known.__name__.upper(), known.rewrite.__code__.co_argcount - 2)
        ret = known.rewrite(cont, form, *args)
        # _debug_printf("REWROTE: %s  ->\n%s", _pp_consly(form, max_depth = 10), _pp_consly(ret[1][1], max_depth = 10))
        return ret

def _rewrite(cont: "Form -> ({} Form Bool)", form) -> "({} Form Bool)":
        ## Unregistered issue CONTINUATION-PASSING-MULTIPLIES-STACK-PRESSURE
        # _debug_printf("REWRITE: %s", _pp_consly(form, max_depth = 10))
        def do_rewrite(form):
                xformed_again, brf = _rewrite_1(cont, form)
                return (do_rewrite(brf[1]) if xformed_again else
                        brf)
        return do_rewrite(form)

def _rewriter_xform(form, further: "Form -> ({} Form Bool)") -> "({} Form Bool)":
        return _rewrite(further, form)

def _rewrite_all(sex, lexenv = nil):
        return _walk_with_lexenv(_rewriter_xform, sex, lexenv = lexenv)
...
def _variable_tn(sym):         return p.name(_unit_variable_pyname(sym))
def _function_tn(sym):         return p.name(_unit_function_pyname(sym))
def _variable_tn_no_unit(sym): return p.name(_ensure_variable_pyname(sym))
def _function_tn_no_unit(sym): return p.name(_ensure_function_pyname(sym))

def _gensym_tn(x = "G"):
        sym = gensym(x)
        sym.tn = _variable_tn(sym)
        return sym
...
_intern_and_bind_symbols("APPLY", "FUNCALL")

def _ir_funcall(func, *args):
        l, l_ = list_, list__
        return l(__apply, l(__function, (l(__quote, l(func)) if isinstance(func, str) else
                                         func)),
                 *(args + (l(__quote, nil),)))

def _ir_cl_call(name, *args):
        return _ir_funcall(list_(__quote, list_("cl", name)), *args)
...
_intern_and_bind_symbols("LET", "&BODY")
def _run_tests_metasex():
        ...
        assert _runtest(just_match,
                        (_consify_star(let, ((__car, ()),
                                             (__cdr, (__car,))),
                                        _body),
                         (let, ({"bindings":[((_satisfies, _namep), (_form,))]},),
                           {"body":[(_form,)]})),
  ...
  &BODY)""",
        ...
        assert _runtest(empty_cross,
                        (nil,
                         ({"a":[(_satisfies, _namep)]}, {"b":[((_satisfies, _namep),)]},)),
                        ({ 'a': nil, 'b': nil },
                         "NIL",
                         None),
                        printer = printer)
        ...
        _intern_and_bind_symbols("PI")
        assert _runtest(simplex,
                        (list_(nil, pi),
                         ({'head':[nil]}, {'tail':(_satisfies, _namep)})),
                        ({ 'head': list_(nil),
                           'tail': pi },
                         "(NILPI)",
                         None),
        ...
        assert _runtest(mid_complex,
                        (list_(pi, list_(pi), list_(pi), list_(pi, pi), list_(pi, pi, pi),
                         ...
                         ({"headname":(_satisfies, _namep)},
                          {"headtupname":((_satisfies, _namep),)},
                                   {"varitupseq":[((_satisfies, _namep), [(_satisfies, _namep)])]},
                                                            {"fix1tupseq":[((_satisfies, _namep),)]},
                                                                                   {"nameseq":[(_satisfies, _namep)]},
                                                                                        {"tailname":(_satisfies, _namep)})),
                        ({ 'headname': pi,
        ...
        assert _runtest(simple_maybe,
                        (list_(pi, __car, __cdr),
                         ({"pi":(_maybe, (_satisfies, _namep))}, {"car":(_satisfies, _namep)}, (_maybe, {"cdr":(_satisfies, _namep)}))),
...
def _handle_linear_body(xs):
        return (nil   if not xs       else
                xs[0] if len(xs) is 1 else
                list_(progn, *xs))

def _rewrite_linear_body(xs):
        return ((nil, list_(progn, *xs)) if len(xs) > 1 else
                (t, _handle_linear_body(xs)))

def _handle_constant_linear_body(xs):
        return (nil    if not xs       else
                xs[-1])
...
@defknown((ir_args, "\n", (_form,), ["\n", (_cons, (_typep, str), (_form, (_for_not_matchers_xform, identity, _metasex_pp)))],))
class ir_args(known):
        def rewrite(cont, orig, form, *args):
                return form[0].rewrite(cont, orig, *_vectorise_linear(form[1]), **{ k: v for k, v in args })
...
def _ir(*ir, **keys):
        ...
        return (list__(ir_args, _consify_linear(ir), _consify_linear([ [k, v] for k, v in keys.items() ])) if keys else
                _consify_linear(ir))
...
class funcall(known):
        def rewrite(cont, _, func, *args):
                return t, cont(list__(__apply, _rewrite(func), append(_xmap_to_conses(_rewrite, args), list_(list_(quote, nil)))))
...
@defknown(((_binder, let_),
           let_, " ",  ([(_notlead, "\n"), (_bind, variable, (_or, (_satisfies, _namep), ((_satisfies, _namep), " ", (_bound, (_form,)))))],),
           1, [(_notlead, "\n"), (_bound, (_form,))]))
class let_(known):
        def rewrite(cont, _, bindings, *body):
                ## I quietly hope that there is no need to handle lexenvs here..
                if not bindings:
                        return t, cont(_handle_linear_body(body))
                if not bindings[1]:
...
@defknown(((_binder, flet),
           flet, " ", ([(_notlead, "\n"), (_bind, function,
                                           ((_satisfies, _namep), (_funcher, lambda_)))],),
           1, [(_notlead, "\n"), (_bound, (_form,))]))
class flet(known):
        def binder_args(bindings, *body):
                return _xmap_to_vector(lambda b: _fn(b[0], b[1][0]), bindings)
        def rewrite(cont, _, bindings, *body):
                l = list_
                return t, cont(_handle_constant_linear_body(body) if all(constantp(x) for x in body) else
                               ## This weak attempt above screams for proper liveness analysis.
                               nil                                if not (bindings or body)          else
                               _handle_linear_body(body)          if not bindings                    else
                               l(funcall,
                                 (lambda rest, tns:
                                          l(labels, _xmap_to_conses(lambda btn:
                                                                            cons(btn[0], btn[1]),
                                                                    zip(tns, bindings)),
                                            l(labels, _xmap_to_conses(lambda btn:
                                                                              l(btn[1][0], l(_rest, rest),
                                                                                l(__apply, l(function, btn[0]), rest)),
                                                                      zip(tns, bindings)),
                                              *body)))
                                 (gensym("REST-"),
                                  (gensym("FN-" + symbol_name(x[0]) + "-")
                                   for x in _vectorise_linear(bindings)))))

# LABELS

labels = intern("LABELS")[0]

@defknown(((_binder, labels),
           labels, " ", ([(_notlead, "\n"), ((_satisfies, _namep), (_funcher, lambda_))],),
           1, [(_notlead, "\n"), (_form,)]))
class labels(known):
        def binder(exp, further):
                def fail(x): error("Bad LABELS form: %s", _pp_consly(x))
                length(exp) < 2 and fail(exp)
                bindings = exp[1][0]
                not listp(bindings) and fail(exp)
                env = symbol_value(_walker_lexenv_)
                with _progv({ _walker_lexenv_:
                              _make_lexenv(env,
                                           name_funcframe = { name: _function_binding(name, function, _fn(name, lam))
        ...
        def rewrite(cont, _, bindings, *body):
                ...
                return t, cont(_handle_constant_linear_body(body) if all(constantp(x) for x in body) else
                               ...
                               nil                                if not (bindings or body)          else
                               _handle_linear_body(body)          if not bindings                    else
                               l(funcall, _ir(lambda_, nil,
                                              ## We rely on that PRIMITIVE layer will allocate a
                                              ## separate function for statements, thereby not wronging the namespace.
                                              *(_xmap_to_vector(lambda binding:
                                                                        _ir(lambda_, *_vectorise_linear(binding[1]),
                                                                            name = binding[0]),
                                                                bindings)
                                                + _vectorise_linear(body)))))

# MACROLET

## Unregistered Issue EXTENDED-LAMBDA-LIST-DESTRUCTURING-WRECKS-ALL
macrolet = intern("MACROLET")[0]

@defknown(((_binder, macrolet),
           macrolet, " ", ([(_notlead, "\n"),
                            (_bind, macro,
                             ((_satisfies, _namep), " ",
                              ([(_notlead, " "),
                                (_or, (_satisfies, _lambda_word_p),
                                 (_bind, variable, (_satisfies, _namep)),
                                 (_bind, variable, ((_satisfies, _namep), (_bound, (_form,)))),
                                 (_form,))],),
                              1, [(_notlead, "\n"), (_form,)]))],),
           1, [(_notlead, "\n"), (_bound, (_form,))]))
...
symbol_macrolet = intern("SYMBOL-MACROLET")[0]

@defknown(((_binder, symbol_macrolet),
           symbol_macrolet, " ", ([(_notlead, "\n"), (_bind, symbol_macro, ((_satisfies, _namep), " ", (_form,)))],),
           1, [(_notlead, "\n"), (_bound, (_form,))]))
...
block = intern("BLOCK")[0]

@defknown(((_binder, block),
           block, " ", (_bind, block, (_satisfies, _namep)),
           [1, (_bound, (_form,))],))
class block(known):
        def rewrite(cont, orig, name, *body):
                ...
                with _progv({ _walker_lexenv_: _make_lexenv(symbol_value(_walker_lexenv_),
                                                            name_blockframe = { name: None for name, _ in bindings }) }):
                        return t, cont(catch_target if has_return_from else _handle_linear_body(body))

# RETURN-FROM

@defknown((intern("RETURN-FROM")[0], " ", (_satisfies, _namep), (_maybe, " ", (_form,))))
...
_intern_and_bind_symbols("%NXT-LABEL", "TAGBODY")
...
class tagbody(known):
        def binder(exp, further):
                binder.tags   = [ gensym("INIT-TAG-")
                                  ] + [ x for x in _vectorise_linear(exp[1])
                ...
                with _progv({ _walker_lexenv_: _make_lexenv(env,
                            ...
                            { _walker_binder_args_: nil }):
                        return further(exp)
        def rewrite(cont, _, *tags_and_forms):
                binder       = _walker_binder()
                ...
                def lam_(seq):
                        ...
                        return l(l_(fun_names[label], nil,
                                    nconc(subseq(body, 0, nlposn),
                                          l(_ir_funcall(fun_names[nextl]) if nlposn else
                ...
                #         `(,init-tag ,@body))
                form = l(let, l(l(go_tag, l(__apply, l(function, __list), l(quote, nil), l(quote, nil)))),
                         l(let, l_(l(return_tag, l(__apply, l(function, __list), l(quote, nil), l(quote, nil))),
                           ...
                           l(catch, return_tag,
                             l(labels, funs,
                               l(let, l(l(_nxt_label, l(function, funs[0][0]))),
                                 l(protoloop,
                                   l(setq, _nxt_label,
                                           l(catch, go_tag, l(__apply, _nxt_label, l(quote, nil))))))))))
                return t, cont(form)

# GO

_intern_and_bind_symbols("GO")

@defknown((__go, " ", (_typep, symbol_t)))
class go(known):
...
_intern_and_bind_symbols("PROGN")
...
when EVAL-WHEN appears as a top level form."""
        def rewrite(cont, _, when, *body):
                ...
                return t, cont(cons(progn, _consify_linear(body)) if exec else
                               nil)

# SETQ

@defknown((intern("SETQ")[0], [" ", (_satisfies, _namep), " ", (_form,)]))
class setq(known):
        def rewrite(cont, orig, *args):
                ## Actually a normalisation.. or actually, the hell knows what it is..
                len(args) % 2 and \
                    error("SETQ accepts an even amount of arguments, got: %s", _pp_consly(_consify_linear(args)))
                not all(isinstance(x, symbol_t) for x in args[::2]) and \
                    error("SETQ arguments at even positions must be symbols, got: %s", _pp_consly(_consify_linear(args)))
                return ((nil, cont(orig)) if len(args) == 2 else
                        (t,   cont(_handle_linear_body(map(lambda name, value: list_(setq, name, value),
                                                           args[::2], args[1::2])))))
        ...
        def lower(name, value):
                ...
                if not lexical_binding or lexical_binding.kind is special:
                        ...
                        return p.special_setq(p.name(_unit_symbol_pyname(name)), _primitivise(value))
                ...
                return p.assign(lexical_binding.tn, _primitivise(value))
...
class progn(known):
        def rewrite(cont, _, *body):
                rewrote, form = _rewrite_linear_body(body)
        ...
        def lower(*body):
                return (p.progn(*(_primitivise(x) for x in body)) if body else
                        _primitivise(nil))
...
class if_(known):
        ...
        def lower(test, consequent, antecedent):
                return p.if_(_primitivise(test),
...
@defknown(((_binder, let),
           let, " ",  ([(_notlead, "\n"), (_bind, variable, (_or, (_satisfies, _namep), ((_satisfies, _namep), " ", (_form,))))],),
           1, [(_notlead, "\n"), (_bound, (_form,))]))
class let(known):
        def rewrite(cont, orig, bindings, *body):
                names, forms = zip(*_xmap_to_vector(lambda x: ((first(x), second(x)) if consp(x) else (x, nil)),
                                                    bindings))
                _check_no_locally_rebound_constants(names)
                return (not (body and bindings and every(lambda x: consp(x) and length(x) == 2, bindings)),
                        cont(_handle_linear_body(body)                if not bindings else
                             list__(progn, append(forms, list_(nil))) if not body     else
                             list_(let, _consify_linear(map(list_, names, forms)))))
        ...
        def lower(bindings, *body):
                ...
                env = _make_lexenv_varframe(list(zip(*normalised))[0], allocate_tns = t)
                return p.let(list((tn, _primitivise(form))
        ...
        def effects(bindings, *body):
                ...
                return any(_ir_effects(f) for f in (x[1][0] for x in bindings) + body)
        def affected(bindings, *body):
                return any(_ir_affected(f) for f in (x[1][0] for x in bindings) + body)

# FUNCTION

_intern_and_bind_symbols("SETF")

@defknown((intern("FUNCTION")[0], " ", (__form,)))
class function(known):
        def rewrite(cont, orig, x):
                lambdap = _pyref_p(x) or not (consp(x) and x[0] is lambda_)
        ...
        def lower(name):
                ...
                return (_function_tn(name) if not lexical_binding else
                        lexical_binding.tn)
...
class unwind_protect(known):
        ...
        def lower(form, *unwind_body):
                return p.unwind_protect(_primitivise(form),
...
class ref(known):
        ...
        def lower(name):
                ...
                if not lexical_binding or lexical_binding.kind is special:
                        ...
                        return p.special_ref(p.name(_unit_symbol_pyname(name)))
...
# aokposp, aokpos, naokpos, wholeposp and aokpos - naokpos)
        ...
        bad_paramspecs = [ x for x in total if not _namep(x) ]
        if bad_paramspecs:
...
@defknown((__lambda, (_funcher, __lambda)))
class lambda_(known):
        def binder(exp, further, name = nil, decorators = nil):
                length(exp) < 2 and error("Bad LAMBDA form: %s", _pp_consly(exp))
                lam = exp[1][0]
                not listp(lam) and error("Bad lambda list in LAMBDA form: %s", _pp_consly(exp))
                clambda = _compiler_lambda(name, lambda_list)
                env = symbol_value(_walker_lexenv_)
                with _progv(dict([ (_walker_binder_args_, nil),
                                   (_walker_lexenv_,
                                    _make_lexenv(env, clambda = clambda,
                                                      name_funcframe = { name: _function_binding(name, function, _fn(name, lambda_list)) }))
                                 ])):
                        return further(exp)
        def rewrite(cont, orig, lambda_list, *body, name = nil, decorators = nil):
                ...
                rewritep = not not (optional or keys or aux)
                if not rewritep: ## Only has &WHOLE and fixed args:
                        with _progv({ _walker_lexenv_: _make_lexenv_varframe(clambda.total, parent = env) }):
                                return nil, cont(orig)
                ## Optimisations
                ...
                return t, \
                    cont(_ir(lambda_, a(l(whole) if whole else nil,
                                        _consify_linear(fixed),
                                        l(_rest, rest_gsym) if need_rest else nil),
                             l(let_, a(_consify_linear(l(name, l(if_, l(prim, p.eq, gs, l(ref, l(quote, l("None")))),
                                                                 def_expr,
                                                                 gs))
                               ...
                               *body),
                             **_dictappend({ "name":       name }       if name       else {},
        ...
        def lower(lambda_list, *body, name = nil, decorators = nil):
                ...
                (whole, fixed, optional, rest, keys, aux), (optdefs, keydefs, auxforms) = \
                    args, forms = clambda.args, clambda.forms
                assert not (whole or optional or keys or aux)
                fnname_tn = _function_tn(name) if name else None
                ## Things, that are needed:
                ##  - CLAMBDA passing
                nonlocal_decl = ([ p.nonlocal_([ _variable_tn(x)
                                                 for x in sorted(clambda.nonlocal_setqs, key = symbol_name) ]) ]
                                 if clambda.nonlocal_setqs else [])
                with _progv({ _lexenv_: _make_lexenv(symbol_value(_lexenv_), clambda = clambda,
                                                     name_varframe  = dict((b, _variable_binding(b, variable, None))
                                                                           for b in clambda.total),
                                                     name_funcframe = { name: _function_binding(name, function, _fn(name, lambda_list)) }) }):
                        return p.lambda_(_lower_lambda_list("LAMBDA", *(fixed, [], rest or None,
                                                                        [], [], [])),
...
class apply(known):
        ...
        def lower(func, arg, *args):
                ...
                if rest is nil or rest == list_(quote, nil):
                        return p.funcall(_primitivise(func), *(_primitivise(x) for x in fixed))
                else:
                        return p.apply(_primitivise(func), *(list(_primitivise(x) for x in fixed)
...
@defknown((intern("QUOTE")[0], " ", (_form, (_for_not_matchers_xform, identity, _metasex_pp))))
class quote(known):
        def nvalues(_):            return 1
        def nth_value(n, orig, _): return orig if n is 0 else nil
        def lower(x):
                ...
                if isinstance(x, symbol_t) and not constantp(x):
                        ...
                        return p.symbol(_unit_symbol_pyname(x))
                else:
                        prim, successp = _try_primitivise_constant(x)
                        if successp:
                                ...
                                return p.literal_list(*(_primitivise(list_(quote, x)) for x in x))
                        else:
...
class multiple_value_call(known):
        ...
        def lower(fn, *arg_forms):
                ...
                return p.apply(_primitivise(fn),
...
class catch(known):
        ...
        def lower(tag, *body):
                return p.catch(_primitivise(tag),
...
class throw(known):
        ...
        def nth_value(n, orig, _, value): return (list_(progn, tag, _ir_nth_value(value)) if _ir_effects(tag) else
                                                  _ir_nth_value(value))
        def lower(tag, value):
                return p.throw(_primitivise(tag), _primitivise(value))
...
class nth_value(known):
        ...
        def lower(n, form):
                return p.funcall(p.impl_ref("_values_frame_project"), _primitivise(n), _primitivise(form))
...
class progv(known):
        ...
        def lower(vars, vals, *body):
                return p.progv((_primitivise(x) for x in vars), (_primitivise(x) for x in vals),
                               p.progn(*(_primitivise(x) for x in body)))
...
class protoloop(known):
        ...
        def lower(*body):
                return p.loop(p.progn(*(_primitivise(x) for x in body)))
...
def _run_tests_known():
        def applyification(input):
                return _compiler_macroexpand_all(input, lexenv = null)
        assert _runtest(applyification,
                        list_(__car),
                        _consify_star(__apply, (function, __car), (quote, nil)))
...
def _report(macroexpanded = None, known = None, primitive = None, ast = None, bytecode = None,
            desc = "", form_id = None):
        desc = "%s - " % desc if desc is not None else ""
        fid = ("  %x" % form_id) if form_id else ""
        if macroexpanded is not None:
                _debug_printf(";;; %smacroexpanded ............%s\n%s\n",
                              desc, fid, _pp(macroexpanded))
        if known is not None:
                _debug_printf(";;; %sknowns ..............%s\n%s\n",
                              desc, fid, _pp(known))
...
def _primitivise(form, lexenv = nil) -> p.prim:
        ...
        def rec(x):
                ...
                if listp(x):
                        ...
                        if isinstance(x[0], symbol_t):
                                ...
                                return known.lower(*_vectorise_linear(form[1]), **_alist_hash_table(args))
                        elif (consp(x[0]) and x[0] and x[0][0] is lambda_):
                                return rec(append(list__(__apply, x), list_(list_(quote, nil))))
        ...
        with _progv({ _lexenv_: _coerce_to_lexenv(lexenv) }):
...
def _lower(form, lexenv = nil):
        ...
        expanded = _rewrite_all(form, lexenv = lexenv)    ## No other high-level entry point to %REWRITE-ALL
        if symbol_value(_compiler_trace_rewrites_):
                _report(known =  expanded, form_id = id(form), desc = "%LOWER")
        ## HIR -> LIR
        prim = _primitivise(expanded, lexenv = lexenv)    ## No other high-level entry point to %PRIMITIVISE.
...
def _process_to_ast(form, lexenv = nil):
        ...
        macroexpanded = _compiler_macroexpand_all(form, lexenv = lexenv)
...
## _read_function_object_ast_compile_load_and_pymport, _compile_in_lexenv (<-= _compile), _compile_in_lexenv (<-= macro_function, _simple_eval)
...
def _compilation_unit_prologue(funs, syms, gfuns, gvars):
        ...
        def symbol_prologue():
                ...
                with _progv(_compiler_debugless_traceless_frame):
                ...
                def in_compilation_unit():
                        ...
                        try:
                                ...
                                while form is not input_file:
                                        ...
                                        ## Beacon LEXENV-CLAMBDA-IS-NIL-HERE
                                        form_stmts = _process_top_level(form, lexenv = _the_null_lexenv())
...
def _read_function_as_toplevel_compile_and_load(body):
        ...
        bytecode = _process_as_loadable(_process_top_level, form, lexenv = _the_null_lexenv(), id = "LISP-")
...
def _compile_in_lexenv(lambda_expression, lexenv = nil, name = None, globalp = None, global_macro_p = None):
        if lexenv and globalp:
                error("In %%COMPILE-IN-LEXENV: the provided non-NULL lexenv conflicts with GLOBALP.")
        form = _ir(*_vectorise_linear(lambda_expression),
                   name = name,
                   **({ "decorators": list_(_ir_cl_call("_set_macro_definition",
                                            ...
                                            _ir_cl_call("_set_function_definition",
                                                        _ir_funcall("globals"), name, lambda_expression))}
                      if globalp else {}))
        bytecode = _process_as_loadable(_linearise_processor(_process_to_ast), form, lexenv = lexenv, id = "COMPILED-LAMBDA-")
        function, bad_gls, good_gls = _load_module_bytecode(bytecode, func_name = name, filename = "<lisp core>")
        bad_gls.update(good_gls)      ## Critical Issue NOW-WTF-IS-THIS-SHIT?!
...
and true otherwise."""
        ...
        return _compile_in_lexenv(lambda_expression,
                                  name           = name,
...
#                 (__apply, (function, (quote, ("cl", "_compiler_defun"))),
#                  (quote, (comma, name)), (quote, (lambda_, (comma, lambda_list), (splice, body))),
#                  (quote, nil))),
#               (eval_when, (_load_toplevel, _execute),
#                 (__apply, (__apply, (function, (quote, ("cl", "_set_function_definition"))),
#                          (quote, (comma, name)), (quote, (lambda_, (comma, lambda_list), (splice, body))),
#                          (quote, nil)),
#                         (progn,
...
def _load_as_source(stream, verbose = nil, print = nil):
        ...
        def with_abort_restart_body():
                ...
                form = next()
                if pathname:
                        with _progv({ _source_info_: nil # _source_info_type(pathname = pathname)
                                     }):
                                while form != stream:
                                        eval_form(form, nil)
                                        form = next()
                else:
                        with _progv({ _source_info_: nil }):
                                while form != stream:
                                        eval_form(form, nil)
                                        form = next()
...
def load(pathspec, verbose = None, print = None,
        ...
        def load_stream(stream, faslp):
                with _progv({ _readtable_:     _symbol_value(_readtable_),
                             _package_:       _symbol_value(_package_),
...
def describe(object, stream_designator = None):
        ...
        with _progv({_print_right_margin_: 72}):
...
ENSURE-GENERIC-FUNCTION."""
        maybe_gfun = _defaulted(_global(the(symbol_t, function_name),
                                        _defaulted(globals, _py.globals())),
                                nil)
