...
def with_symbol_unit_magic(body, machine = None, standalone = nil, id = "UNIT-"):
        ...
        def in_compilation_unit():
                ...
                return ((machine.compilation_unit_prologue(*unit_data) if standalone else []) +
...
def compile_in_lexenv(lambda_expression, lexenv = nil, name = None, globalp = None, global_macro_p = None, macroexpand = t):
        ...
        if symbol_value(_compiler_trace_forms_):
                dprintf(";;;%s compiling:\n%s%s",
                              sex_space(-3, ";"), sex_space(), pp(form))
        def process_to_ast(form, lexenv = None, macroexpand = None):
                return lower(compiler_macroexpand_all(form, lexenv = lexenv, desc = "PROCESS-TO-AST") if macroexpand else
                             form,
                             lexenv = lexenv,
                             machine = pymach)
        stmts, *_ = with_symbol_unit_magic(lambda: process_to_ast(form, lexenv = lexenv, macroexpand = macroexpand),
                                           standalone = t, id = "COMPILED-LAMBDA-")
        bytecode = assemble(stmts, form)
