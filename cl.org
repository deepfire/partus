...
def __cl_condition_handler__(condspec, frame):
        ...
        with progv({_stack_top_hint_: signalling_frame}):
                cond = _sys.call_tracing(continuation, tuple())
        if type_of(cond) not in __not_even_conditions__:
        ...
        def rec(x):
                return (x.n                               if isinstance(x, _ast.Num)   else
                        ...
                        _intern(_frost.python_name_lisp_symbol_name(x.id))[0]
                                                          if isinstance(x, _ast.Name)  else
...
_ast_info = _poor_man_defstruct("_ast_info",
                                ...
                                "bound_free",
                                "nfixed")
__ast_walkable_field_types__ = set([_ast.stmt, (pylist, _ast.expr), (maybe, _ast.expr),
...
#+begin_src python +n
  def defast(fn):
          ...
          def validate_defast_lambda_list(ast_type, lambda_list, annotations):
                  ...
                  type_specifier_type = (or_, pytuple, type, pytypename)
...
def _ast_FunctionDef(name:            string,
                     ...
                     decorator_list: (pylist, _ast.expr) = list(),
...
def _ast_Name(id:  string,
              ctx: _ast.expr_context):
        def bound_free(): ((set(), set([id])) if typep(ctx, (or_, _ast.Load, _ast.AugLoad, _ast.Param)) else
                           (set([id]), set()))
  ...
  class _metasex_matcher(_matcher):
          ...
          def preprocess(m, pat):
                  ...
                  return ((_count_scope, (some,) +
                           m.preprocess(tuple(pat))) if typep(pat, list)       else
...
def apply():
        ...
        def lower(func, arg, *args):
                def pycall_p(x): return stringp(x) or (_tuplep(x) and all(stringp(x) for x in x))
...
def _parse_eval_when_situations(situ_form):
        if not (_tuplep(situ_form) and set(situ_form) == _eval_when_keywords):
