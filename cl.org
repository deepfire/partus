...
******* IR-BINDS
        ...
        ..but we can't do that before all macroexpansion is complete.. and we need the IR-BINDS
        for the macroexpander to process lexenv changes due to macrolet/symbol-macrolet..

        Cold version IR-BINDS is due?
...
********* Deviations from CL lambda list
          ...
          - the name bound by &REST does not contain the arglist which is then (optionally
            partially) destructured by the &KEY-bound part of the parameter list.
