...
* Chapter 3: Still better life (+ condition system)
...
***** MetaSEX
      ...
      - atom :: any of the following:
        - string :: separator specifier, only for pretty-printer
        - integer :: separator specifier, only for pretty-printer
        - METASEX:NAME :: a non-keyword symbol
        - COMMON-LISP:KEYWORD :: a keyword symbol
        - METASEX:FORM :: arbitrary form;  a walkable
        - COMMON-LISP:STRING :: a string
        - any other symbol :: itself, non-walkable
      - tuple :: complex form
        - (METASEX:FIXED, ...) :: tuple with a fixed structure
        - (METASEX:ANY, ...) :: choice
        - (COMMON-LISP:SOME, ...) :: arbitrary repetition of contained structure
        - (METASEX:MAYBE-ONCE, ...) :: marker for an element of (COMMON-LISP:SOME, (METASEX:ANY, ...))
        - (METASEX:ONCE, ...) :: marker for an element of (COMMON-LISP:SOME, (METASEX:ANY, ...))
      - list :: syntactic sugar for COMMON-LISP:SOME
      - dictionary :: bindings, available either at toplevel, or at iteration of depth one
      ...
      Example code for DEFGENERIC:

      #+begin_example
      (_fixed, defgeneric, " ", (_any, _name, (_fixed, setf, " ", _name)), " ", #<DEFGENERIC-LAMBDA-LIST>,
        1, (_some_of_any,
             ## Unregistered Issue METASEX-HOW-TO-CUSTOM-DEFGENERIC-OPTIONS
             {"methods": (_fixed, _keyword("METHOD"), " ", [_keyword, " "], #<DEFMETHOD-LAMBDA-LIST>,
                           1, [_form, "\n"]), "\n"},
...
******* Additional requirements
        ...
        - choice (lambda list defaults, DEFINE-METHOD-COMBINATION)
        - specifiers for known types
          - keyword
          - non-keyword symbols ("name")
        - specifiers for atoms from arbitrary sets
