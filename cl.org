...
def _defscope(name, var, **initargs):
        def make_frame(bindings):
                return _py.tuple(bindings.items())
        def push_frame(**bindings):
                return _dynamic_scope_push({ var: (make_frame(bindings), symbol_value(var)) })
        def pop_frame(*_): _dynamic_scope_pop()
        return _defwith(name, push_frame, pop_frame())
...
***** Specification
      ...
      - set specifier :: any of the following, within approprite context:
        - symbol :: any of the following:
          ...
          - otherwise, a set of a single element denoting self
...
#+begin_src python +n
  ...
  def _metasex(top_sex, initial_depth = None):
          ...
          def code_interp_rec(sex, code, continue_ = nil):
                          ...
                          with progv({_sex_justification_: _symbol_value(_sex_justification_) + increment}):
                                  ...
                                  def horz_run(code, sex):
                                          def horz_rec(acc, code, sex):
                                                  ...
                                                  if _listp(code[0]):
                                                          return acc + code_interp_rec(sex, code[0]), 0, nil, nil
                                                  ...
                                                  if symbolp(sex) or symbolp(code):
                                                          warn(simple_warning,
                                                               ...
                                                               top_sex)
                                                          return acc, 0, nil, nil
                                                  sub = code_interp_rec(sex[0], code[0])
                                  ...
                                  while code:
                                          ...
                                          acc += sub + separatorp("\n" if sex else "")[0]
                                  return acc
                  def fail():                   return "#<INVALID-SEX>", True
                  def repr_if_or_fail(pred, x): return (x.__repr__(), None) if pred(x) else fail()
                  ### code_interp_rec()
                  if _tuplep(code):
                          def metasex_interp_fixed(sex, code):
                                  return "(" + structure_interp(sex, code, increment = 1) + ")"
                          ...
                          def metasex_interp_set_minus(sex, code):
                                  if not code: err_bad_code((_set_minus,) + code)
                                  set, *subtractae = code
                                  def setspec_interp(x):
                                          # Set specifier bindings not supported just yet.
                                          ((if hasattr(x, "python_type") else)
                                           if symbolp(x) else
                                           ())
                                  return
                          ...
                          return _metasex(sex)
                  elif code is _name:   return repr_if_or_fail(stringp, sex)
                  elif code is keyword: return repr_if_or_fail(keywordp, sex)
                  elif code is string:  return repr_if_or_fail(stringp, sex)
                  else: #Other symbols:
                          return repr_if_or_fail(lambda x: x is code, sex)
