...
***** Dynamic scope
      ...
      - we assume that the same python runtime is available
      - therefore, importing cl, and calling runtime-provided cl.__dynamic_scope__ manipulation
...
#+begin_src python
  ...
  def _r(x, y, retval, q = "", n = 20, ignore_callers = set(["<lambda>", "complex"])):
          def trace_args():
                  frames = reversed(
                          list(_take(n, (f for f in _frames_calling(_caller_frame()) if _frame_fun_name(f) not in ignore_callers))))
                                                  ...
                                                  ((lambda seg_bound, seg_r, seg_fail_pat:
                                                   ...
                                                   (*(m.succ(m.bind((), bound, name), m.prod((), orifst[0])) if seg_exp == () else
                                                      ...
                                                      m.match(bound, name,  seg_exp,     aux,  (False,
                                                                                                firstp), aux, (limit - 1 if isinstance(limit, int) else
                                                                                                               None))))),
                                          lambda seg_bound:
  ...
  def _match(matcher, exp, pat):
          ...
          return matcher.match(dict(), name, exp, prepped, (True, False), None, None)
...
class _lexenv():
        ...
        def lookup_var_kind(self, kind, x, default = None):
                ...
                return (b and b.kind is kind and b) or default
def _lexenvp(x):         return isinstance(x, _lexenv.python_type)
...
def _expand_quasiquotation(quasiquotation_form):
        ...
        def process_qq(x):
                if atom_pyfun(x):
                        ...
                        if all(consp(x) and x[0] is list for x in acc[1:]):
                                acc = (list,) + tuple(x[1] for x in acc[1:])
                        return tuple(acc)
  ...
  """
          def do_defknown(fn, sym, pyname, metasex):
                  ...
                  def meth(name, body, *args):
                          return (name, lambda *_: compile(body % args, "", "exec",
...
#+begin_src python +n
  _intern_and_bind_names_in_module("%NAME", "%MAYBE", "%NOTLEAD", "%NOTTAIL",
  ...
  def _form_known(form):
          ...
          complex_form_p = isinstance(form, tuple) and symbolp(form[0])
  ...
  class _metasex_matcher(_matcher):
          ...
          def preprocess(m, pat):
                  ...
                  return ((_count_scope, (some,) +
                           m.preprocess(tuple(pat))) if isinstance(pat, list)       else
          ...
          def form_metasex(form):
                  ...
                  return ((_typep, t)                             if not isinstance(form, tuple)                                      else
                          ()                                      if not form                                               else
          ...
          def form(m, bound, name, form, pat, orifst):
                  ...
                  _trace_printf("form", "=== form for %s:\n    %s", lambda: (repr(form), prepped))
                  return _r(form, pat, m.match(bound, name, form, prepped, (isinstance(form, tuple), orifst[1]), None, None))
  ...
  class _metasex_matcher_pp(_metasex_matcher):
          ...
          def prod(x, orig_tuple_p):
                  result = str(x) if x or orig_tuple_p else ""
...
#+begin_src python
  ...
  # def alternates():
  #         return _match(_metasex_pp, (1, "a"), {"whole":([(or_, (_typep, int),
  #                                                               (_typep, str))],)})
...
#+begin_src python +n
  ...
  def _macroexpander_inner(m, bound, name, form, pat, orifst, compilerp = t):
          ...
          known = _find_known(expanded_form[0]) if isinstance(expanded_form, tuple) else nil
          (symbol_frame,
           mfunc_frame,
...
******* LET, etc
        ...
        #                                       [ car_pyfun(x) for x in bindings_thru_defaulting ],
...
def _ir_prepare_lispy_lambda_list(lambda_list_, context, allow_defaults = None, default_expr = None):
        ...
        if (restposp and keyposp and (keypos - restpos != 1) or
            ...
            restkeyposp and (len(lambda_list_) - restkeypos != 2)):
        ...
        _keys = list(lambda_list_[keypos + 1:restkeypos or None]) if keypos else tuple()
        ...
        optional = list(lambda_list_[optpos + 1:restpos or keypos or restkeypos or None]) if optposp else []
...
def _lower_name(name, ctx = "Load"):
        ...
        if isinstance(name, tuple) and ctx != "Load":
                error("COMPILE-NAME: only 'Load' context possible while lowering (SYMBOL ..) forms.")
        return ("Name", _frost.full_symbol_name_python_name(name[1] if isinstance(name, tuple) else name), (ctx,))
        ...
        def lower(x):
                ...
                if symbolp(x) and not constantp(x):
                        ...
                        if successp:
                                ...
                                return (apply, (function, list)) + tuple((quote, x) for x in x) + (nil,)
...
def if_():
        def nvalues(test, consequent, antecedent):
                ...
                return (nconseq                 if nconseq == nante                      else
                        max(nconseq, nante) if isinstance(nconseq, int) and isinstance(nante, int) else
                        t)
        ...
        def lower(test, consequent, antecedent):
                ...
                if all([cons_expr_p, ante_expr_p]):
                        ...
                        return (flet, cons_fdefn + ante_fdefn,
                                 (if_, test, cons, ante))
        def effects(test, consequent, antecedent):  return any(_ir_effects(f) for f in [test, consequent, antecedent])
        def affected(test, consequent, antecedent): return any(_ir_affected(f) for f in [test, consequent, antecedent])
#+end_src
...
def let():
        ...
        def lower(bindings, *body):
                ...
                if every(_tuple_expression_p, compiled_value_pves):
                        ...
                        form = ((progn,) +
                                tuple((setq, n, v) for n, v in zip(temp_names, values[:n_nonexprs])) +
                                (_ir(lambda_, (_optional,) + tuple(zip(names, temp_names + values[n_nonexprs:])), *body,
...
def flet():
        ...
        def lower(bindings, *body):
                ...
                form = ((let, tuple((name, (progn,
                                                 (def_, tempname, lambda_list) + tuple(fbody),
                                                 (ref, tempname)))
                                        for tempname, (name, lambda_list, *fbody) in zip(tempnames, bindings))) +
                        body)
...
def nth_value():
        ...
        def nth_value(n, orig, form_n, form):
                return ((nth_value, n, orig) if not (isinstance(n, int) and isinstance(form_n, int)) else ## Give up.  Too early?
...
def def_():
        ...
        def lower(name, lambda_list, *body, decorators = []):
                ...
                with progv({_compiler_def_:        cdef,
                        ...
                        xtnls_guess, xtnls_actual, try_ = None, set(), 0
        ...
        def lower(lambda_list, *body, dont_delay_defaults = nil):
                ...
                if body_exprp:
                        ...
                        if not (optional or keys):
                                ...
                                return (lambda_, tuple(fixed + [_optional] + optional + keys),
...
def apply():
        ...
        def lower(func, arg, *args):
                def pycall_p(x): return isinstance(x, str) or (isinstance(x, tuple) and all(isinstance(x, str) for x in x))
                ## Unregistered Issue IMPROVEMENT-APPLY-COULD-VALIDATE-CALLS-OF-KNOWNS
                with _no_tail_position():
                        func_pro, func_val = (([], _attr_chain_atree((func,) if isinstance(func, str) else func))
                ...
                if every(_tuple_expression_p, arg_pvs):
                        ...
                        temp_names = tuple(_gensyms(n = len(arg_pvs) - no_varargs, x = "APPLY-ARG"))
                        arg_exprs = temp_names + ((nil,) if no_varargs else ())
                        return (let, func_binding + tuple(zip(temp_names, args)),
        ...
        def _rec(x):
                ...
                if listp(x): ## nil or _tuplep
                        ...
                        def puntedp(x):
                                ...
                                return x and isinstance(x, tuple) and symbolp(x[0])
...
def lisp(body):
        ...
        form = _read_ast(body_ast[0])
        __def_allowed_toplevels__ = set([defun, defmacro])
...
def _compile_toplevel_def_in_lexenv(name, form, lexenv, globalp = nil, macrop = nil, lambda_expression = None):
        ...
        def _in_compilation_unit():
                ...
                if len(pro) != 1:   # The FunctionDef and the symbol Assign
...
def _preprocessor_macroexpand_1(form):
        def _macroexpand_1(form, env):
                ...
                #  (values form nil))
                return (macroexpand_1(form, env) if atom_pyfun(form) or not (symbolp(car_pyfun(form)) and special_operator_p(car_pyfun(form)))
...
def _parse_eval_when_situations(situ_form):
        if not (isinstance(situ_form, tuple) and set(situ_form) == _eval_when_keywords):
...
def compile_file(input_file, output_file = None,
        ...
        def _compile_file_default_pathname(input_file, output_file = None):
                if not input_file.endswith(".lisp"):
                        error("In COMPILE-FILE %s: must end with .lisp.", repr(input_file))
...
declared to be the names of constant variables)."""
        return (typep(form, (or_, integer, float, complex, string)) or
                ...
                (isinstance(form, tuple) and len(form) == 2 and form[0] is quote_))
#+end_src
...
def _specializerp(x):       return ((x is t)        or
                                    typep(x, (or_, type, (pytuple, (eql, eql), t))))
...
GENERIC-FUNCTION argument is then returned."""
        ...
        if not generic_function:
                ...
                _frost.setf_global(generic_function, function_name, globals = _defaulted(globals, globals()))
        else:
