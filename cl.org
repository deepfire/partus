...
*** The way
    ...
    - we acutely need multiple values
    - we need preliminary macroexpansion, to get down to just IR knowns and funcalls
      - for that, we need a walker, to macroexpand only form-positions
        - for that, we made MetaSEX
          - but also for: pretty-printing, validation and generally convenient operation
    - further, see Compilation/Compiler/Thoughts on processing
    - maybe we need a binding-less matcher..
...
******* Notes about computation of ()-ness
        ...
        - ()-ness desirability
          - we want ()
            - any tuple, which is completely contained in the SEX
          - () no want
            - any other case
        - ()-ness knowledge
          - top level
