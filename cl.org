...
***** Method descriptions
      ...
      - NTH-VALUE ::
        ...
        - IR of the value form, otherwise
      - BINDS ::
...
***** Known form definitions
      :PROPERTIES:
      :COLUMNS: %40ITEM %IMPL{X} %K{X} %VALUES(Values){X} %EFFECTS(Effects){X} %AFFECTED(Affected){X}
      :END:
******* Optimisation
********* LET, etc
