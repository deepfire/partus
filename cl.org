...
#+TODO:  IMPL CONFLICT DUPLICATION DEAD-CODE ATTIC | OBSOLETE IRRELEVANT DONE
...
***** LET, etc
      ...
      # names = _mapset(_ensure_car, bindings)
      # if _tail_position_p() and not ((_mapsetn(_atre__free, body) - set(names)) or
...
***** LET, FLET, LABELS
      ...
      - warn on/drop dead code, when bound names are not used
...
*** Function definitions: overview (September 24, 2012, 15:44)
    ...
    - _function_scope -- compilation environment
      ...
      - code emitted by _compile_in_lexenv
      - FUNCTION -- the
    - pynames
      - _function_tn, _ensure/unit/get_function_pyname -- toolkit
