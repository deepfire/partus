...
(defun initialize-syntax-table (self &key &allow-other-keys)
...
(defun copy-syntax-table (self)
  (declare (syntax-table self))
  (let ((copy (make-instance 'syntax-table)))
    (initialize-syntax-table copy)
...
(defun character-description ((self syntax-table) (ch character))
  (declare (syntax-table self) (character ch))
...
(defun (setf character-description) (val (self syntax-table) (ch character))
  (declare (syntax-table self) (character ch))
  (let ((code (char-code ch)))
    (if (< code +standard-characters-limit+)
...
(defclass readtable ()
  ((case          :initarg :case
   ...
   (syntax-table  :accessor readtable-syntax-table
                  ...
                  :initform (let ((st (make-instance 'syntax-table)))
                              (initialize-syntax-table st)
                              st))
...
(defun copy-readtable (&optional (from-readtable *readtable*) (to-readtable nil))
  (if (null from-readtable)
      (if (null to-readtable)
          (let ((rt (make-instance 'readtable)))
            (initialize-readtable rt)
            rt)
          (progn
            ...
            (initialize-syntax-table (readtable-syntax-table to-readtable))
            to-readtable))
      (if (null to-readtable)
          (let* ((initargs (list :case (readtable-case from-readtable)
                                 :syntax-table (copy-syntax-table
                                                (readtable-syntax-table from-readtable))
                                 :parse-token (readtable-parse-token from-readtable)))
                 (rt (apply #'make-instance 'readtable initargs)))
            (apply #'initialize-readtable rt initargs)
            rt)
          (progn
...
(defun process-case-function (mode)
  (ecase mode
    (:preserve (function identity))
    (:downcase (function char-downcase))
    (:upcase   (function char-upcase))
    (:invert   (lambda (ch)
...
ONE-OR-MORE and OPT-SIGN."
  (multiple-value-bind (docu decl body) (parse-body body)
...
(defun initialize-readtable (self &rest rest &key &allow-other-keys)
